...
#include "CubeActorRMC.h"

// Sets default values
ACubeActorRMC::ACubeActorRMC() {
	PrimaryActorTick.bCanEverTick = true;

	cube1 = CreateDefaultSubobject<URuntimeMeshComponentStatic>(TEXT("GeneratedMesh"));
	RootComponent = cube1;
}
void ACubeActorRMC::GenerateBoxMesh() {
	...
	CreateBoxMesh(FVector(50, 50, 50), Vertices, Triangles, Normals, TextureCoordinates, Tangents,
		Colors); // Create the mesh section, specifying collision.
	cube1->CreateSectionFromComponents(0, 0, 0, Vertices, Triangles, Normals, TextureCoordinates, Colors, Tangents, ERuntimeMeshUpdateFrequency::Infrequent,
		true);
}
void ACubeActorRMC::CreateBoxMesh(FVector BoxRadius, TArray < FVector >& Vertices, TArray < int32 >
...
void ACubeActorRMC::PostActorCreated()
{
	Super::PostActorCreated();
	GenerateBoxMesh();
}
void ACubeActorRMC::PostLoad()
{
	Super::PostLoad();
	GenerateBoxMesh();
}